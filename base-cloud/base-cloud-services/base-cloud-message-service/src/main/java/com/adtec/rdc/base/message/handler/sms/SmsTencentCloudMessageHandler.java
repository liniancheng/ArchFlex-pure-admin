...
import com.adtec.rdc.base.message.config.TencentCloudPropteriesConfig;
...
public class SmsTencentCloudMessageHandler extends SmsMessageHandler {
	...
	public Boolean execute(MessageTemplate messageTemplate) {
		try {
			...
			SmsMultiSender msender = new SmsMultiSender(tencentCloudPropteriesConfig.getAppId(),
					tencentCloudPropteriesConfig.getAppKey());
			String[] mobile = { smsMessageTemplate.getMobile() };
			Integer templateId = Integer.valueOf(smsMessageTemplate.getTemplate());
			// 签名参数未提供或者为空时，会使用默认签名发送短信
			SmsMultiSenderResult result = msender.sendWithParam("86", mobile, templateId,
					smsMessageTemplate.getParams(), smsMessageTemplate.getSignName(), "", "");
