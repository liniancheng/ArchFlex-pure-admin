package com.adtec.rdc.base.gateway.router;


import com.adtec.rdc.base.gateway.entity.SysZuulRoute;
...
public class DynamicRouteLocator extends SimpleRouteLocator implements RefreshableRouteLocator {
   ...
   private LinkedHashMap<String, ZuulProperties.ZuulRoute> locateRoutesFromDB() {
       ...
       for (SysZuulRoute route: sysZuulRoutes) {
           // 为空跳过
           if (Strings.isNullOrEmpty(route.getRoutePath()) && Strings.isNullOrEmpty(route.getUrlProxy())) {
               continue;
           }
           ...
           try {
               ...
               zuulRoute.setPath(route.getRoutePath());
               zuulRoute.setServiceId(route.getServiceId());
               zuulRoute.setRetryable(Objects.equals("0", route.getRetryableFlag()) ? Boolean.FALSE : Boolean.TRUE);
               zuulRoute.setStripPrefix(Objects.equals("0", route.getStripPrefix()) ? Boolean.FALSE : Boolean.TRUE);
               zuulRoute.setUrl(route.getUrlProxy());
               List<String> sensitiveHeadersList = route.getHeadersList() != null ? Arrays.asList(route.getHeadersList().split(",")) : null;
