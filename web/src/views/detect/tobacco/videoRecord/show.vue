...
import { getRecord as getVideoRecord } from "@/api/detect/video";
import { getRecord as getCameraRecord } from "@/api/detect/camera";
...
const getData = () => {
    ...
    if (state.type == "camera") {
        getCameraRecord(state.id).then(callback);
    } else {
        getVideoRecord(state.id).then(callback);
    }
};

const playVideos = () => {
    const leftVideo = document.querySelector(
        ".left .video"
    ) as HTMLVideoElement;
    const rightVideo = document.querySelector(
        ".right .video"
    ) as HTMLVideoElement;
...
const startDrag = (e: MouseEvent) => {
    ...
    const onMouseMove = (moveEvent: MouseEvent) => {
        ...
        let newLeftWidth =
            startLeftWidth + (deltaX / cardsContainerWidth) * 100;
