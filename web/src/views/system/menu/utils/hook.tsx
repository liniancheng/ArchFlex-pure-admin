...
import { getMenuList, unFormatMenu } from "@/api/system";
            ...
            beforeSure: (done, { options }) => {
                ...
                FormRef.validate(valid => {
                    if (valid) {
                        ...
                        if (title === "新增") {
                            // 实际开发先调用新增接口，再进行下面操作
                            saveAsyncRoutes(unFormatMenu(curData)).then(() => {
                                chores();
                            });
                        } else {
                            // 实际开发先调用修改接口，再进行下面操作
                            updateAsyncRoutes(unFormatMenu(curData)).then(
                                () => {
                                    chores();
                                }
                            );
